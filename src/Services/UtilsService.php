...
class UtilsService
{
    public function limitText($text, $limit) {
        return strlen($text) > $limit ? mb_substr($text, 0, $limit) : $text;
    }

    public function htmlEncodeBeforeSave($html) {
    ...
    public function htmlCut($text, $length = 100, $ending = '...', $exact = true, $considerHtml = false) {
        if (is_array($ending)) {
            extract($ending);
        }
        if ($considerHtml) {
